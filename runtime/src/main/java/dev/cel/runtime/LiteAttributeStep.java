...
public final class LiteAttributeStep {
   ...
   * qualifierLists}.
   */
  public static @Nullable Object qualifyAttribute(
    ...
    for (Object item : qualifierLists) {
      ...
      if (qualifier.size() < 3
          ...
          || !(qualifier.get(2) instanceof Number)) {
      ...
      Step step =
          LiteSelectQualifier.create(
              fieldNumber, fieldName, typeCode, defaultValue, celValueConverter);
  ...
  public static boolean hasField(
    ...
    for (int i = 0; i < size; i++) {
      ...
      String fieldName = (String) qualifier.get(1);
      if (i < size - 1) {
        Step step = LiteSubmessageQualifier.create(fieldNumber, fieldName, celValueConverter);
        obj = step.qualify(obj);
  ...
  private interface Step {
    @Nullable Object qualify(@Nullable Object value);
  }
  ...
  private static final class LiteSelectQualifier implements Step {
    ...
    @Override
    public @Nullable Object qualify(@Nullable Object obj) {
      if (obj == null || obj instanceof NullValue) {
        return defaultValue;
      }
      ...
      if (obj instanceof ProtoMessageLiteValue) {
        ...
        if (!unknowns.isEmpty()) {
          ...
          if (celValueConverter instanceof ProtoLiteCelValueConverter) {
            protoTypeName =
                ((ProtoLiteCelValueConverter) celValueConverter)
                    .findFieldDescriptor(msg.celType().name(), fieldNumber)
                    .map(FieldLiteDescriptor::getFieldProtoTypeName)
                    .orElse(fieldName);
          }
          Object decoded =
              RawProtoMessageLiteValue.decodeWireEntries(
                  unknowns, typeCode, protoTypeName, isRepeated);
          if (decoded != null) {
            return decoded;
          }
        }
        ...
        if (celValueConverter instanceof ProtoLiteCelValueConverter) {
          return ((ProtoLiteCelValueConverter) celValueConverter)
              .findDefaultCelValue(msg.celType().name(), fieldNumber)
              .orElse(defaultValue);
        }
      ...
      if (obj instanceof SelectableValue) {
        ...
        return found.isPresent() ? found.get() : defaultValue;
      }
  ...
  private static final class LiteSubmessageQualifier implements Step {
    ...
    private static LiteSubmessageQualifier create(
        int fieldNumber, String fieldName, CelValueConverter celValueConverter) {
      return new LiteSubmessageQualifier(fieldNumber, fieldName, checkNotNull(celValueConverter));
    }

    private LiteSubmessageQualifier(
        int fieldNumber, String fieldName, CelValueConverter celValueConverter) {
      this.fieldNumber = fieldNumber;
      this.fieldName = checkNotNull(fieldName);
      this.celValueConverter = celValueConverter;
  ...
  private static final class LitePresenceQualifier implements Step {
    ...
    @Override
    public Object qualify(@Nullable Object obj) {
      if (obj == null || obj instanceof NullValue) {
        return false;
      }
