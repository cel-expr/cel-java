...
final class EvalIndex extends PlannedInterpretable {
  ...
  private @Nullable Object maybeEvaluateIndexUnknown(
    ...
    if (targetUnknowns.attributes().isEmpty()) {
      AccumulatedUnknowns merged = AccumulatedUnknowns.maybeMerge(targetUnknowns, indexVal);
      return merged != null ? merged : targetUnknowns;
    }
    ...
    if (targetUnknowns.attributes().size() == 1) {
      ...
      if (attributeResolver != null) {
        ...
        if (resolved.isPresent() && !(resolved.get() instanceof CelUnknownSet)) {
          return enforceStrictnessAndAdaptUnknowns(resolved.get());
        }
      }
      return AccumulatedUnknowns.create(
    ...
    Object firstResolvedVal = null;
    boolean hasConflict = false;
    boolean hasAnyResolved = false;
    ...
    for (CelAttribute attr : targetUnknowns.attributes()) {
      ...
      if (attributeResolver != null) {
        ...
        if (resolved.isPresent() && !(resolved.get() instanceof CelUnknownSet)) {
          if (!hasAnyResolved) {
            firstResolvedVal = resolved.get();
            hasAnyResolved = true;
          } else if (!Objects.equals(firstResolvedVal, resolved.get())) {
            hasConflict = true;
          }
          continue;
        }
    ...
    if (remaining.isEmpty()) {
      if (hasAnyResolved && !hasConflict) {
        return enforceStrictnessAndAdaptUnknowns(firstResolvedVal);
      }
