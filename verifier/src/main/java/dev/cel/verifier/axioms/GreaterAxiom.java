...
/** Axiomatization for CEL's greater operator. */
@SuppressWarnings({"unchecked", "rawtypes"}) // Z3 Java API uses raw types.
final class GreaterAxiom {

  static final CelZ3FunctionAxiom INSTANCE =
      CelZ3FunctionAxiom.newBuilder(StandardFunction.GREATER.functionDecl())
          .addBinaryOverloadTranslator(
              Comparison.GREATER_INT64.celOverloadDecl(),
          ...
          .addBinaryOverloadTranslator(
              Comparison.GREATER_BYTES.celOverloadDecl(),
              (ctx, typeSystem, constraintSink, lhs, rhs) ->
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.MkStringLt(
          ...
          .addBinaryOverloadTranslator(
              Comparison.GREATER_INT64_DOUBLE.celOverloadDecl(),
              (ctx, typeSystem, constraintSink, lhs, rhs) ->
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.mkGt(
                  ...
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.mkGt(
                              ...
                              (ArithExpr) typeSystem.getUint(rhs)))))
          .addBinaryOverloadTranslator(
              Comparison.GREATER_UINT64_INT64.celOverloadDecl(),
              (ctx, typeSystem, constraintSink, lhs, rhs) ->
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.mkGt(
