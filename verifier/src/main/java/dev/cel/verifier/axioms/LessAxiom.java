...
/** Axiomatization for CEL's less operator. */
@SuppressWarnings({"unchecked", "rawtypes"}) // Z3 Java API uses raw types.
final class LessAxiom {

  static final CelZ3FunctionAxiom INSTANCE =
      CelZ3FunctionAxiom.newBuilder(StandardFunction.LESS.functionDecl())
          .addBinaryOverloadTranslator(
              Comparison.LESS_INT64.celOverloadDecl(),
          ...
          .addBinaryOverloadTranslator(
              Comparison.LESS_BYTES.celOverloadDecl(),
              (ctx, typeSystem, constraintSink, lhs, rhs) ->
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.MkStringLt(
          ...
          .addBinaryOverloadTranslator(
              Comparison.LESS_INT64_DOUBLE.celOverloadDecl(),
              (ctx, typeSystem, constraintSink, lhs, rhs) ->
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.mkLt(
                  ...
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.mkLt(
                              ...
                              (ArithExpr) typeSystem.getUint(rhs)))))
          .addBinaryOverloadTranslator(
              Comparison.LESS_UINT64_INT64.celOverloadDecl(),
              (ctx, typeSystem, constraintSink, lhs, rhs) ->
                  Optional.of(
                      typeSystem.wrapBool(
                          ctx.mkLt(
