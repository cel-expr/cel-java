...
final class PrattParser {
  ...
  /** Returns the recorded position of {@code id}, or {@code 0} if it has none. */
  private int getPositionOrZero(long id) {
    return Math.max(getPosition(id), 0);
  }
  ...
  private long copyId(long id) {
    ...
    return nextId(getPositionOrZero(id));
  }
  ...
  private int getLeftmostPosition(CelExpr expr) {
    ...
    return getPositionOrZero(expr.id());
  }
  ...
  private Optional<CelExpr> tryExpandMacro(
    ...
    if (nodeLimitExceeded) {
      reportError(
          getPositionOrZero(exprId), "could not expand macro: expression node limit exceeded");
      return Optional.empty();
    }
    ...
    int macroPosition = getPositionOrZero(exprId);
  ...
  private final class PrattMacroExprFactory extends CelMacroExprFactory {
    ...
    protected CelSourceLocation getSourceLocation(long exprId) {
      return source.getOffsetLocation(getPosition(exprId)).orElse(CelSourceLocation.NONE);
    }
