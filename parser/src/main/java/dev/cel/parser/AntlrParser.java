...
/** ANTLR-based parser implementation for CEL. */
final class AntlrParser extends CELBaseVisitor<CelExpr> {
  ...
  private AntlrParser(
      CelOptions options,
      Collection<CelMacro> macros,
      CelSource.Builder sourceInfo,
      ExprFactory exprFactory) {
    this.options = options;
    this.macros = macros.stream().collect(ImmutableMap.toImmutableMap(CelMacro::getKey, m -> m));
  ...
  public CelExpr visitInt(IntContext context) {
    ...
    try {
      ...
      // Do not propagate e.getMessage(), which is JDK-specific.
      return exprFactory.reportError(
          context, "invalid int literal: %s", context.getText());
    }
  ...
  public CelExpr visitUint(UintContext context) {
    ...
    try {
      ...
      // Do not propagate e.getMessage(), which is JDK-specific.
      return exprFactory.reportError(
          context, "invalid uint literal: %s", context.getText());
    }
  ...
  public CelExpr visitDouble(DoubleContext context) {
    ...
    try {
      ...
      // Do not propagate e.getMessage(), which is JDK-specific.
      return exprFactory.reportError(
          context, "invalid double literal: %s", context.getText());
    }
    return exprFactory.newExprBuilder(context.tok).setConstant(constExpr).build();
  }
