...
public class CelListsExtensionsTest extends CelExtensionTestBase {
  ...
  public void slice_success(String expression, String expected) throws Exception {
    Object result =
        eval(cel, expression, ImmutableMap.of("non_list", ImmutableSortedSet.of(4L, 1L, 3L, 2L)));
  ...
  @TestParameters(
      "{expression: '[timestamp(\"2026-01-01T00:00:00Z\"),"
          + " timestamp(\"2025-01-01T00:00:00Z\")].sort()', expected:"
          + " '[timestamp(\"2025-01-01T00:00:00Z\"), timestamp(\"2026-01-01T00:00:00Z\")]'}")
  ...
  @TestParameters(
      "{expression: '[b\"b\", b\"a\"].sortBy(e, e)', " + "expected: '[b\"a\", b\"b\"]'}")
  @TestParameters(
      "{expression: '[duration(\"2s\"), duration(\"1s\")].sortBy(e, e)', "
          + "expected: '[duration(\"1s\"), duration(\"2s\")]'}")
  @TestParameters(
      "{expression: '[timestamp(\"2026-01-01T00:00:00Z\"),"
          + " timestamp(\"2025-01-01T00:00:00Z\")].sortBy(e, e)', expected:"
          + " '[timestamp(\"2025-01-01T00:00:00Z\"), timestamp(\"2026-01-01T00:00:00Z\")]'}")
  @TestParameters(
