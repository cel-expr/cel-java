...
public final class CelStringExtensionsTest extends CelExtensionTestBase {
  ...
  public void lastIndexOf_strLengthLessThanSubstrLength_returnsMinusOne(
      @TestParameter({"@@", "  ", "a", "abc", "나", "😁"}) String lastIndexOf)
      throws Exception {
    Object evaluatedResult =
  ...
  @TestParameters("{expr: \"'%s'.format([0.000000002])\", expectedResult: '2.0E-9'}")
  @TestParameters("{expr: \"'%s'.format([[0.000000002]])\", expectedResult: '[2.0E-9]'}")
  @TestParameters("{expr: \"'%.5s'.format(['foobar'])\", expectedResult: 'fooba'}")
  @TestParameters("{expr: \"'%.3s'.format(['foobar'])\", expectedResult: 'foo'}")
  @TestParameters("{expr: \"'%.0s'.format(['foobar'])\", expectedResult: ''}")
  @TestParameters(
      "{expr: \"'%.10s'.format(['foobar'])\", expectedResult: 'foobar'}") // Precision larger than
  // length
  ...
  @TestParameters(
      "{expr: \"'%s'.format([{1: 'a', '1': 'b'}])\", expectedResult: '{1: a, 1: b}'}") // Both
  // format to
  // "1", but
  // both are
  // present
  ...
  @Test
  @Ignore("Tests recursion limits of the evaluation engine itself")
  public void format_cyclicMap_success() throws Exception {
    Map<Object, Object> cyclicMap = new HashMap<>();
    cyclicMap.put("self", cyclicMap);
    Object evaluatedResult = eval("'%s'.format([dynMap])", ImmutableMap.of("dynMap", cyclicMap));
    assertThat(evaluatedResult).isEqualTo("{self: {...}}");
  }

  @Test
  @Ignore("Tests recursion limits of the evaluation engine itself")
  public void format_cyclicList_success() throws Exception {
    List<Object> cyclicList = new ArrayList<>();
    cyclicList.add(cyclicList);
    Object evaluatedResult = eval("'%s'.format([dynList])", ImmutableMap.of("dynList", cyclicList));
