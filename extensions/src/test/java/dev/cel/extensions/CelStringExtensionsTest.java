...
public final class CelStringExtensionsTest extends CelExtensionTestBase {
  ...
  @Test
  public void format_verbS_bytesWithMaxSingleByteCodePoint_advancesOneByte() throws Exception {
    // U+007F (DEL) is the largest code point encoded in a single UTF-8 byte, so the decoder must
    // advance exactly one byte past it. The leading invalid byte forces the lossy decoding path,
    // and the trailing 'A' is dropped if U+007F is mistakenly treated as a two-byte sequence.
    Object evaluatedResult = eval("'%s'.format([b'\\xff\\x7fA'])");

    assertThat(evaluatedResult).isEqualTo("\uFFFD\u007FA");
  }
  ...
  @TestParameters("{expr: \"'%d %d %d'.format([0, 1])\", expectedMessage: 'index 2 out of range'}")
  ...
  @TestParameters(
      "{expr: \"'%b'.format(['foo'])\", expectedMessage: 'binary clause can only be used on"
          + " integers and bools, was given string'}")
  @TestParameters(
      "{expr: \"'%d'.format(['foo'])\", expectedMessage: 'decimal clause can only be used on"
          + " numbers, was given string'}")
  @TestParameters(
      "{expr: \"'%o'.format(['foo'])\", expectedMessage: 'octal clause can only be used on"
          + " integers, was given string'}")
  @TestParameters(
      "{expr: \"'%x'.format([3.14])\", expectedMessage: 'hex clause can only be used on integers,"
          + " byte buffers, and strings, was given double'}")
  @TestParameters(
      "{expr: \"'%f'.format(['foo'])\", expectedMessage: 'fixed point clause can only be used on"
          + " doubles, integers, and unsigned integers, was given string'}")
  @TestParameters(
      "{expr: \"'%e'.format(['foo'])\", expectedMessage: 'scientific clause can only be used on"
          + " doubles, integers, and unsigned integers, was given string'}")
  // The reported type is the CEL type, not the underlying Java class, so that the message matches
  // what the Go and C++ runtimes emit.
  @TestParameters(
      "{expr: \"'%d'.format([null])\", expectedMessage: 'was given null_type'}")
  @TestParameters(
      "{expr: \"'%d'.format([duration('30m2s')])\", expectedMessage: 'was given"
          + " google.protobuf.Duration'}")
  @TestParameters(
      "{expr: \"'%d'.format([timestamp('2025-01-01T00:00:00Z')])\", expectedMessage: 'was given"
          + " google.protobuf.Timestamp'}")
  @TestParameters("{expr: \"'%d'.format([[1, 2]])\", expectedMessage: 'was given list'}")
  @TestParameters("{expr: \"'%d'.format([{1: 2}])\", expectedMessage: 'was given map'}")
  @TestParameters("{expr: \"'%o'.format([b'foo'])\", expectedMessage: 'was given bytes'}")
  @TestParameters("{expr: \"'%b'.format([3.14])\", expectedMessage: 'was given double'}")
  @TestParameters("{expr: \"'%x'.format([true])\", expectedMessage: 'was given bool'}")
